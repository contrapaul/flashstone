...
- [ ] **1.1** Show the balance in the nav, next to the account. Read-only, from
      `/api/profile`.
      → **verify:** it updates after a reward without a page reload.

- [ ] **1.2** **Daily login bonus, 50 gold**, on the first login of a UTC calendar day.
...
- [ ] **1.3** **Win rewards: 25 vs AI, 40 online.** The AI award is the awkward one — the
...
- [ ] **2.1** `db/migrations/0002_quests.sql` — append-only, do not edit `0001`:
      ```sql
      CREATE TABLE quests (
...
- [ ] **2.2** Quest definitions in `src/lib/quests/quests.ts` — id, label, target, reward,
...
- [ ] **2.3** Progress is reported by the client to `POST /api/quests/progress` as
...
- [ ] **2.5** Quest panel UI: 3 cards with progress bars, a claim button on completion,
      and a countdown to refresh.
      → **verify:** complete one end to end and see the gold land.

## 3. The shop

- [ ] **3.1** `/shop` route: packs (100 gold) and card backs (300 gold). Balance shown,
...
- [ ] **3.2** `POST /api/shop/buy-pack`: debit and deal **in a single D1 transaction/batch**,
...
- [ ] **4.1** `src/lib/packs/pack.ts` — pure, seeded, unit-testable. Rules from
...
- [ ] **4.1b** **Gold variants** (`DECISIONS.md` §10). After a pack's 5 cards are chosen,
...
- [ ] **4.2** Pack generation runs **on the server**, using the seeded RNG in
...
- [ ] **4.3** Opening UI: **5 card backs facing the player; each flips on click or tap.**
...
- [ ] **4.4** Handle the **completion edge**: when every card is owned at 2 copies, say so
...
- [ ] **5.1** Card backs are art files under `static/art/backs/` per the Phase 1 §5 spec,
      registered the same way — dropping a file in adds a back.
      → **verify:** add a placeholder file; it appears in the shop.

- [ ] **5.1b** The shop sells **card backs only** — gold cards are not purchasable
...
- [ ] **5.2** Ship **4**: 1 default (owned by everyone) and 3 at 300 gold. Selection is
...
- [ ] **5.3** `CardBack.svelte` takes a back id and renders its art, falling back to

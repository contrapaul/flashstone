...
//
// Ported from the `time` repo. The SQL is unchanged; only the throw is, since
// SvelteKit's `error()` replaces the Pages Functions HttpError.
export async function rateLimit(env: any, key: string, max: number, windowMs: number): Promise<void> {
  ...
  const row = await env.DB.prepare(
    ...
    .first();
  if (row && (row.count as number) > max) {
    error(429, 'Too many attempts. Please wait a while and try again.');
  }
}
